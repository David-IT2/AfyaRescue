...
<div class="max-w-xl">
    ...
    <form method="post" action="{{ $ambulance ? route('super-admin.ambulances.update', $ambulance) : route('super-admin.ambulances.store') }}" class="space-y-4 rounded-lg border bg-white p-4 shadow">
        ...
        <div>
            <label class="block text-sm font-medium text-slate-700">Hospital</label>
            <select name="hospital_id" required
                class="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-slate-900 bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                @foreach($hospitals as $h)
                    <option value="{{ $h->id }}" {{ old('hospital_id', $ambulance?->hospital_id) == $h->id ? 'selected' : '' }}>{{ $h->name }}</option>
                @endforeach
            </select>
            @error('hospital_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
        </div>
        ...
        <div>
            ...
            <select name="driver_id"
                ...
                @foreach($drivers as $d)
                    <option value="{{ $d->id }}" {{ old('driver_id', $ambulance?->driver_id) == $d->id ? 'selected' : '' }}>{{ $d->name }} ({{ $d->hospital_id ? 'H#' . $d->hospital_id : '—' }})</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="plate_number" value="{{ old('plate_number', $ambulance?->plate_number) }}"
                class="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-slate-900 bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="latitude" value="{{ old('latitude', $ambulance?->latitude) }}"
                    class="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-slate-900 bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
            ...
            <div>
                ...
                <input type="text" name="longitude" value="{{ old('longitude', $ambulance?->longitude) }}"
                    class="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-slate-900 bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
        ...
        <div class="flex gap-2">
            <button type="submit" class="rounded-md bg-red-600 px-4 py-2 text-white hover:bg-red-700">Save</button>
            <a href="{{ route('super-admin.ambulances.index') }}" class="rounded-md border border-slate-300 px-4 py-2 text-slate-700 hover:bg-slate-50">Cancel</a>
        </div>

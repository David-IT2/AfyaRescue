...
<div class="max-w-xl">
    ...
    <form method="post" action="{{ $hospital ? route('super-admin.hospitals.update', $hospital) : route('super-admin.hospitals.store') }}" class="space-y-4 rounded-lg border bg-white p-4 shadow">
        ...
        <div>
            <label class="block text-sm font-medium text-slate-700">Level (1-3)</label>
            <input type="number" name="level" min="1" max="3" value="{{ old('level', $hospital?->level ?? 1) }}"
                class="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-slate-900 bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('super-admin.hospitals.index') }}" class="rounded-md border border-slate-300 px-4 py-2 text-slate-700 hover:bg-slate-50">Cancel</a>
        </div>
    </form>
</div>
@endsection